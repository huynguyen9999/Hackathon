...
export function ProgramVariantsCard({ variants }: ProgramVariantsCardProps) {
  ...
  return (
    <div className="card-glow rounded-2xl border border-indigo-500/25 bg-slate-900/50 p-6">
      <h2 className="text-lg font-semibold text-slate-50">Combined degree pathways</h2>
        ...
        {variants.map((variant) => (
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h3 className="text-sm font-semibold text-indigo-100">{variant.label}</h3>
                <p className="mt-0.5 text-xs text-slate-500">{variant.degree}</p>
              </div>
              {variant.url ? (
                <a
                  href={variant.url}
                  ...
                  className="text-xs font-medium text-teal-300 hover:text-indigo-200"
                >
                  Program page ↗
                </a>
              ) : null}
