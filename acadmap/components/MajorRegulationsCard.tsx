...
export function MajorRegulationsCard({ regulations }: MajorRegulationsCardProps) {
  return (
    <div className="rounded-2xl border border-amber-500/20 bg-amber-950/15 p-6">
      <h2 className="text-lg font-semibold text-slate-50">Major regulations</h2>
      <dl className="mt-4 grid gap-3 sm:grid-cols-2">
        {regulations.pre_major_gpa != null && (
          <>
            <dt className="text-xs font-semibold uppercase tracking-wider text-amber-200/80">
              Pre-major GPA
            </dt>
        ...
        {regulations.pnp_allowed === false && (
          <>
            ...
            <dd className="text-sm text-slate-300">Not allowed for major courses</dd>
          </>
        )}
      </dl>
        ...
        <div className="mt-5">
          ...
          <ul className="mt-2 space-y-1.5">
            {regulations.transfer_admission_rules.map((rule) => (
              <li
                key={rule}
                className="text-sm text-slate-300 before:mr-2 before:text-amber-400 before:content-['•']"
              >
                {rule}
              </li>
            ))}
        ...
        <div className="mt-5">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {regulations.excluded_courses.map((code) => (
              <span
                key={code}
                className="rounded bg-slate-800/80 px-2 py-0.5 font-mono text-[11px] text-slate-400"
              >
                {code}
              </span>
            ))}
