    ...
    <div>
      ...
      <ul className="space-y-1.5">
        {items.map((item) => (
          <li
            key={item}
            className="flex gap-2 text-sm text-slate-300 before:content-['•'] before:text-indigo-400"
          >
            <span>{item}</span>
          </li>
        ))}
  ...
  return (
    <div className="card-glow rounded-2xl border border-indigo-500/20 bg-slate-900/50 p-6">
          ...
          {major.sample_electives.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              {major.sample_electives.map((code) => (
                <span
                  key={code}
                  className="rounded bg-indigo-500/15 px-2 py-0.5 font-mono text-[11px] text-indigo-200"
                >
                  {code}
                </span>
              ))}
      ...
      {major.career_outcomes.length > 0 && (
        <div className="mt-6">
          <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-violet-300/80">
