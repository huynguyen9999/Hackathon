...
export function LsFrameworkCard({ framework }: { framework: LsRequirementsFramework }) {
  return (
    <section className="rounded-2xl border border-teal-500/20 bg-slate-50 dark:bg-slate-900/50 p-6">
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wider text-teal-700 dark:text-teal-300/80">
      ...
      <div className="mt-6">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {framework.ge_special_subject_areas.map((item) => (
            <span
              key={item}
              className="rounded-full border border-teal-500/25 bg-teal-950/30 px-3 py-1 text-xs text-teal-900 dark:text-teal-100"
            >
              {item}
            </span>
          ))}
