/**
 * Validates Berkeley coe-majors detail files against coe-catalog.json slugs and schema depth.
 * Run: npm run validate:berkeley-details
 */
...
function seedPathForSlug(slug: string): string {
  return path.join(SEEDS_DIR, `berkeley-${slug}.json`);
}
...
function validateDetailFile(file: string) {
  ...
  const strictParity = expectedSlug === "eecs";
  ...
  if (expectedSlug === "eecs") {
    if (detail.recommended_plans.length !== 8) {
      fail(`${file}: expected 8 semester plan entries, got ${detail.recommended_plans.length}`);
    }
    if (!hasSeed) {
      fail(`${file}: missing seed file berkeley-${expectedSlug}.json for roadmap parity`);
    }
  }
}
...
async function main() {
  ...
  console.log(`Validated ${slugs.size} Berkeley coe-majors detail files.`);
}
